...
import {
    Table, Tag, Button, Input, Select, Tabs, Badge, Tooltip, Modal, Form, Space,
    Popconfirm, message,
} from 'antd';
import {
...
function TopicManagementPage() {
    ...
    const fetchSemesters = useCallback(async () => {
        try {
            ...
            if (res.success) setSemesters(res.data.map(s => ({ value: s.id, label: s.name })));
        } catch { /* pass */ }
    }, []);

    useEffect(() => { fetchTopics(); fetchSemesters(); }, [fetchTopics, fetchSemesters]);
    ...
    const openFormModal = (topic = null) => {
        ...
        if (topic) { form.setFieldsValue({ title: topic.title, description: topic.description, maxStudents: topic.maxStudents, semesterId: topic.semesterId }); }
        else { form.resetFields(); form.setFieldsValue({ maxStudents: 1, semesterId: 1 }); }
        setFormModalOpen(true);
    };

    const handleSaveDraft = async () => {
        try { const values = await form.validateFields(); setSubmitting(true);
            if (editingTopic) { await topicService.update(editingTopic.id, { ...values, status: 'DRAFT' }); message.success('Đã cập nhật bản nháp.'); }
            else { await topicService.create({ ...values, status: 'DRAFT' }); message.success('Đã lưu bản nháp đề tài.'); }
            setFormModalOpen(false); fetchTopics();
        } catch (err) { if (err?.message) message.error(err.message); } finally { setSubmitting(false); }
    };

    const handleCreateApproved = async () => {
        try { const values = await form.validateFields(); setSubmitting(true);
            if (editingTopic) { await topicService.update(editingTopic.id, { ...values, status: 'PENDING' }); message.success('Đã gửi đề tài đi phê duyệt.'); }
            else { await topicService.create({ ...values, status: 'APPROVED' }); message.success('Tạo đề tài thành công.'); }
            setFormModalOpen(false); fetchTopics();
        } catch (err) { if (err?.message) message.error(err.message); } finally { setSubmitting(false); }
    };

    const handleApprove = async (id) => {
        try { await topicService.changeStatus(id, { status: 'APPROVED' }); message.success('Đã duyệt đề tài.'); fetchTopics(); }
        catch (err) { message.error(err?.message || 'Không thể duyệt đề tài.'); }
    };

    const handleReject = async () => {
        if (!rejectReason.trim()) { message.warning('Vui lòng nhập lý do từ chối.'); return; }
        try { setSubmitting(true); await topicService.changeStatus(rejectingTopicId, { status: 'REJECTED', rejectReason });
            message.success('Đã từ chối đề tài.'); setRejectModalOpen(false); setRejectReason(''); fetchTopics();
        } catch (err) { message.error(err?.message || 'Không thể từ chối đề tài.'); } finally { setSubmitting(false); }
    };

    const handleDelete = async (id) => {
        try { await topicService.delete(id); message.success('Xóa đề tài thành công.'); fetchTopics(); }
        catch (err) { message.error(err?.message || 'Không thể xóa đề tài.'); }
    };

    const handleViewDetail = async (id) => {
        try { const res = await topicService.getById(id); setDetailTopic(res.data); setDetailModalOpen(true); }
        catch { message.error('Không thể xem chi tiết đề tài.'); }
    };
    ...
    const columns = [
        ...
        { title: 'Người đề xuất', dataIndex: 'proposedBy', key: 'proposedBy', render: (u) => <span className="text-sm">{u?.fullName || '—'}</span> },
        {
            title: 'Đã ĐK', key: 'registered', width: 80, align: 'center',
        ...
        },
        {
            ...
            render: (status) => { const cfg = statusConfig[status]; return <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-bold ${cfg?.tw || 'bg-slate-100 text-slate-600'}`}>{cfg?.label || status}</span>; },
        },
        {
        ...
        <div className="py-2">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4">
                    <Select value={statusFilter === 'all' && activeTab === 'all' ? 'all' : statusFilter}
                        onChange={(v) => { setStatusFilter(v); setActiveTab('all'); }} style={{ width: 180 }}
                        options={[{ value: 'all', label: 'Tất cả trạng thái' }, { value: 'DRAFT', label: 'Bản nháp' }, { value: 'PENDING', label: 'Chờ duyệt' }, { value: 'APPROVED', label: 'Đã duyệt' }, { value: 'REJECTED', label: 'Từ chối' }]}
                    />
                    <Input placeholder="Tìm đề tài..." prefix={<SearchOutlined />} value={searchText} onChange={(e) => setSearchText(e.target.value)} style={{ width: 320 }} allowClear />
                </div>

                <Table dataSource={topics} columns={columns} loading={loading}
            ...
            {/* Modal tạo/sửa */}
            <Modal title={editingTopic ? 'Chỉnh sửa bản nháp' : 'Thêm đề tài mới'} open={formModalOpen} onCancel={() => setFormModalOpen(false)}
            ...
            {/* Modal từ chối */}
            <Modal title="Từ chối đề tài" open={rejectModalOpen} onCancel={() => { setRejectModalOpen(false); setRejectReason(''); }}
                onOk={handleReject} confirmLoading={submitting} okText="Từ chối" okButtonProps={{ danger: true }}>
